...
static char gs_print_queue_buf[10240];
/**
 *  \brief 加入打印对列
 *  \param const char *
 *  \param ...
...
void add2print_queue(const char *fmt, ...)
{
    ...
    sprintf(&gs_print_queue_buf[strlen(gs_print_queue_buf)], buf, strlen(buf));

    printf("add gs_print_queue_buf=%s\n", gs_print_queue_buf);
}
...
{
    int len = strlen(gs_print_queue_buf);

    if (0 == len)
        return NULL;

    // strcpy(out, gs_print_queue_buf);
    // printf("get gs_print_queue_buf=%s\n", gs_print_queue_buf);
    sprintf(out, "%s", gs_print_queue_buf);

    memset(gs_print_queue_buf, 0, sizeof(gs_print_queue_buf));

    return out;
}

const u32_t CRC32_Tbl[256] = {
...
/// 获取 CRC32 位校验值
/// </summary>
/// <param name="buf"></param>输入数据数组首地址
/// <param name="size"></param>数据长度
/// <param name="CRC"></param>初始 CRC,初始值为 0xffffffff
/// <returns></returns>返回检验值
u32_t Get_CRC32(u08_t *buf, u32_t size, u32_t CRC)
{
    u32_t i, crc_v = 0;
    crc_v = CRC;
    for (i = 0; i < size; i++)
        crc_v = CRC32_Tbl[(crc_v ^ buf[i]) & 0xff] ^ (crc_v >> 8);
    return crc_v ^ 0xFFFFFFFF;
}
